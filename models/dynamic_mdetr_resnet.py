...
class DynamicMDETR(nn.Module):
    ...
    def forward(self, img_data, text_data):
        ...
        pseudo_class_emb = self.pseudo_class_embedding.weight.unsqueeze(1).repeat(1, bs, 1) # Learnable Token
        
        vl_src = torch.cat([visu_src, text_src, pseudo_class_emb], dim=0)
        pseudo_mask = torch.zeros(bs, 1).to(text_mask.device)
        vl_mask = torch.cat([visu_mask, text_mask, pseudo_mask], dim=1)
        ...
        pseudo_class_feat = vl_feat[self.num_visu_token + self.num_text_token:]
